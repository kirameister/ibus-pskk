...
class TestGetDictionaryFiles:
    ...
    def test_includes_system_dictionary_when_exists(self, temp_dirs, sample_dictionary):
        ...
        # Create system dictionary in config_dir (not dict_dir)
    ...
    def test_includes_extended_dictionary_when_exists(self, temp_dirs, sample_dictionary):
        ...
        with open(ext_dict_path, 'w', encoding='utf-8') as f:
            json.dump(sample_dictionary, f)
        ...
        assert len(result) == 1
        assert result[0] == ext_dict_path

    def test_loads_config_automatically_when_not_provided(self, temp_dirs, sample_dictionary):
        """Test that config is not needed - function just checks for known files"""
        os.makedirs(temp_dirs['config_dir'], exist_ok=True)

        # Create system dictionary in config_dir
        system_dict_path = os.path.join(temp_dirs['config_dir'], 'system_dictionary.json')
        with open(system_dict_path, 'w', encoding='utf-8') as f:
            json.dump(sample_dictionary, f)
